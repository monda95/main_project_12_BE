...
- 채팅 인터페이스 스타일(`static/css/chat.css`)을 전역 토큰(`static/css/style.css`) 참조 방식으로 전환해 다크 모드·라이트 모드 동기화와 섀도/색상 일관성을 확보함.【F:static/css/chat.css†L1-L120】【F:static/css/style.css†L1-L170】
...
- 채팅 화면은 토큰 기반 스타일로 통합됐으므로, 실제 대화 흐름에서 라이트/다크 모드·입력/응답 상태별 미세 UI를 QA 캡처와 함께 검증해야 함.【F:static/css/chat.css†L1-L160】
