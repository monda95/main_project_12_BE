...
| DH_RQ04IDEMP | 멱등성(전처리 한정) | 비기능 | 전처리 Job 생성 시 서버가 dataset_id + owner_id + steps 기반으로 `client_job_id`를 자동 생성하며, 클라이언트는 지정하지 않는다. 동일 요청은 기존 Job을 반환한다. (201 Created / 200 OK) | 중 | UNIQUE(dataset_id, client_job_id) |
