...
| client_job_id | VARCHAR(64)| NULL, UNIQUE(dataset_id, client_job_id) WHERE client_job_id IS NOT NULL | 멱등키(요청 기반 자동 생성, 데이터셋별 유일) | NULL     |
| status        | VARCHAR(20)| NOT NULL, DEFAULT 'queued', CHECK IN('queued','running','succeeded','failed') | 상태                        | 'queued' |
| steps         | JSONB      | NULL                                                                 | 전처리 단계 정의              | NULL     |
| created_at    | TIMESTAMPTZ| NOT NULL DEFAULT now()                                                | 생성 시각                    | now()    |
...
- **idx_prejobs_dataset (dataset_id)**  
---
